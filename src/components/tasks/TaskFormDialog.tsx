...
export function TaskFormDialog({ open, onOpenChange, task }: TaskFormDialogProps) {
  ...
  const { user } = useAuth();
  const [taskType, setTaskType] = useState<'personal' | 'assignment' | 'admin_task'>('personal');
  ...
  const [assignmentType, setAssignmentType] = useState<'user' | 'section' | 'class'>('user');
        ...
        <form onSubmit={onSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-4">
            ...
            {taskType === 'admin_task' && user?.role === 'admin' && (
              <div className="space-y-2">
                <Label htmlFor="teacherId">Assign to Teacher <span className="text-red-500">*</span></Label>
                <Select 
